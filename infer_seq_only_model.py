...
# ——— Ablation Function ———
def ablate_channels(X, channels):
    X = X.clone()
    X[:, channels, :] = 0
    return X
...
X = ablate_channels(X, list(range(0, 11)))  # ablate channels 0–10
...
for fold in range(1, 6):
    ...
    ckpt = torch.load(f'weights/best_seq_only_model_{fold}th.pth', map_location=device)
